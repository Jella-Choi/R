...
### 표본의 크기가 클 때
## 신뢰구간
# 예제. 봉지에 과자를 담는 두 대의 기계 A와 B가 있는데, 이 두 기계가 동일한 양의 과자를 담는지 확인하고자 한다. 
# A 기계에서 50, B기계에서 100봉지의 과자 무게를 재본 결과 평균이 각각 453 g, 401 g이고 표준편차가 각각 80 g과 60 g으로 나왔다. 
# 두 기계가 과자를 담는 양의 차이의 95% 신뢰구간을 구하라.
n_A <- 50;n_A
n_B <- 100;n_B
xbar_A <- 453;xbar_A
xbar_B <- 401;xbar_B
...
abs(Z) >= z_0.005
# 검정 결과 계산된 검정통계량이 기각역에 포함되므로 신뢰수준 99% 하에서 귀무가설을 기각할 수 있다.
# 따라서 두 기계가 채워 넣는 과자의 평균무게에 차이가 있다고 할만한 충분한 근거가 있다.[반드시 해석까지 써야함]
...
T >= t_0.05
# 검정 결과 계산된 검정통계량은 기각역에 포함되지 않으므로 신뢰수준 95% 하에서 귀무가설을 기각할 수 없다.
# 따라서들판에서 말린 목초가 인공적으로 말린 목초보다 우유를 더 많이 생산하게 하는 사료라고 말할만한 근거가 충분치 않다

#### 표본의 크기가 작을 때
### 두 모집단의 모표준편차가 다를 때
## 신뢰구간
# 예제. 지방의 한 도시에서 작은 개천을 사이에 두고 남북으로 나뉘어 있는 두 지역의 집값을 비교하고자 
# 최근에 매매가 이루어진 집을 대상으로 남쪽에서 13가구, 북쪽에서 11가구의 집값을 조사하였더니 다음과 같았다. 95% 신뢰구간을 구하여라.
n_A <- 13;n_A
n_B <- 11;n_B
xbar_A <- 2.4;xbar_A
xbar_B <- 2.15;xbar_B
...
T = ((xbar_A-xbar_B)-delta)/se;T
abs(T) >= t_0.025
# 검정 결과 계산된 검정통계량이 기각역에 포함되지 않으므로 신뢰수준 95% 하에서 귀무가설을 기각할 수 없다.
# 두 지역 간에 집값에 차이가 있다고 할만한 충분한 근거가 없다.
